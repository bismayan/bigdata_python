...
abstract class TweetSet {
  ...
  //= {
//    if (p==true) acc.union(this) else this
//  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
 ...
 def descendingByRetweet: TweetList= {
  ...
  def mostRetweeted:Tweet= {
   val l = try left.mostRetweeted catch {
     case e: NoSuchElementException => new Tweet(" ", " ", -1)
   }
   val r = try right.mostRetweeted catch {
     case e: NoSuchElementException => new Tweet(" ", " ", -1)
   }
   if (l.retweets > elem.retweets)
     if (r.retweets > l.retweets) r else l
   else if (r.retweets > elem.retweets) r else elem
 }
